...
// functions for printing float and doubles
void parse_double(unsigned long long double_as_int, unsigned char *sign_field, unsigned short *exponent_field,
...
void parse_float(unsigned long long float_as_int, unsigned char *sign_field, unsigned short *exponent_field,
                 unsigned int *fraction_field)
{
    // float is 1 sign bit | 8 exponent bits | 23 fraction bits

    *sign_field = (unsigned char)(float_as_int >> (31));
    *exponent_field = (unsigned short)(float_as_int >> (23) & 0xFF);
    // get last 23 bits;
    *fraction_field = float_as_int & 0x007FFFFF;
}

int sprint_raw_float_hex(char *dst, unsigned long long float_as_int)
{
    // @todo: implement NaN, infinity etc.
    unsigned char sign_field;
    unsigned short exponent_field;
    unsigned int fraction_field;
    parse_float(float_as_int, &sign_field, &exponent_field, &fraction_field);
    if (sign_field == 1) {
        dst[0] = '-';
        dst++;
    }
    return sprintf(dst, "0x1.%06.6Xp%+df", fraction_field, exponent_field - 127);
}

int sprint_raw_double_hex(char *dst, unsigned long long double_as_int)
{
    if (sizeof(double) == 4) {
        return sprint_raw_float_hex(dst, double_as_int);
    } else {
        unsigned char sign_field;
        unsigned short exponent_field;
        unsigned long long fraction_field;
        parse_double(double_as_int, &sign_field, &exponent_field, &fraction_field);
        if (sign_field == 1) {
            dst[0] = '-';
            dst++;
        }
        return sprintf(dst, "0x1.%013.13llXp%+d", fraction_field, exponent_field - 1023);
    }
}
...
#  endif
     ...
     /* zero padding to specified minimal field width? */
      ...
      case 'a': {
          // double
//          double double_arg = va_arg(ap, double);
//          str_arg_l = sprint_raw_double_hex(tmp, double_arg);
          double double_arg = va_arg(ap, unsigned long long);
          if (length_modifier == '2') {
              str_arg_l = sprint_raw_double_hex(tmp, double_arg);
          } else {
              // float
              str_arg_l = sprint_raw_float_hex(tmp, double_arg);
          }

//          double double_arg = va_arg(ap, double);
//          str_arg_l = sprintf(tmp, "%a", double_arg);
          str_arg = tmp;
          zero_padding = 0;
